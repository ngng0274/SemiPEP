...
class PairData(PygData):
    def __inc__(self, key: str, value: Any, *args, **kwargs) -> Any:
        if key == "edge_index_b":
            return self.x_b_pre_cal.size(0)
        if key == "edge_index_g":
            return self.x_g_pre_cal.size(0)
        if key == "edge_index_bg":
            return torch.tensor([[self.x_b_pre_cal.size(0)], [self.x_g_pre_cal.size(0)]])
...
class AsEPv1Dataset(PygInMemoryDataset):
    ...
    def get_idx_split(self, split_method: str = None) -> Dict[str, Tensor]:
        ...
        return torch.load(osp.join(self.root, self.name, "split", "split_dict.pt"), weights_only=False)[
            split_method
        ]
...
def load_interim_graph_pt(abdbid: str, interim_graph_dir: str) -> Dict[str, Any]:
    ...
    return torch.load(pt_fp, weights_only=False)
...
def get_node_feat_from_interim_graph_data(
    ...
    # antibody
    # ----------------------------------------
    for t in cfg.node_feat_type:
        if t == "pre_cal":
            node_feats["x_b_pre_cal"] = interim_graph_data["embedding"]["ab"][cfg.ab.embedding_model]
        elif t == "one_hot":
            concat_seq = "{}{}".format(
                interim_graph_data["seqres"]["ab"]["H"],
                interim_graph_data["seqres"]["ab"]["L"],
            )
            node_feats["x_b_one_hot"] = aa_seq_to_one_hot_tensor(aa_seq=concat_seq)
        elif t == "custom":
            node_feats["x_b_custom"] = torch.cat(
                [
    ...
    for t in cfg.node_feat_type:
        if t == "pre_cal":
            node_feats["x_g_pre_cal"] = interim_graph_data["embedding"]["ag"][cfg.ag.embedding_model]
        elif t == "one_hot":
            node_feats["x_g_one_hot"] = aa_seq_to_one_hot_tensor(list(interim_graph_data["seqres"]["ag"].values())[0])
        elif t == "custom":
            node_feats["x_g_custom"] = cfg.ag.custom_embedding_method(list(interim_graph_data["seqres"]["ag"].values())[0])

    # TODO: should we also provide the full version?
    # use the binary vector to slice the embeddings
    for key in ["x_b_pre_cal", "x_b_one_hot"]:
        if key in node_feats:
            node_feats[key] = node_feats[key][(interim_graph_data["mapping"]['ab']['seqres2cdr']).astype(bool)]
    for key in ["x_g_pre_cal", "x_g_one_hot"]:
        if key in node_feats:
            node_feats[key] = node_feats[key][(interim_graph_data["mapping"]['ag']['seqres2surf']).astype(bool)]
...
def convert_one_interim_to_pyg_pair_data(
    ...
    # node feat
    node_feats = get_node_feat_from_interim_graph_data(
    ...
    y_b = torch.zeros(node_feats["x_b_pre_cal"].size(0), dtype=torch.long)
    y_g = torch.zeros(node_feats["x_g_pre_cal"].size(0), dtype=torch.long)
