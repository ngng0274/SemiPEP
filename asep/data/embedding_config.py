...
class EmbeddingConfig(BaseModel):
    ...
    # )
    node_feat_type: List[str] = ["pre_cal"]
    ...
    def check_node_feat_type(cls, v):
        if not isinstance(v, list):
            raise ValueError("node_feat_type must be a list of strings.")
        
        invalid_types = [t for t in v if t not in ALLOWED_NODE_FEAT_TYPES]
        if invalid_types:
            raise ValueError(f"Invalid node_feat_type values: {invalid_types}. Must be among {ALLOWED_NODE_FEAT_TYPES}")
    ...
    # def adjust_embedding_model(cls, values):
    #     node_feat_type = values.get("node_feat_type")
    #     if "one_hot" in node_feat_type:
    #         values["ab"].embedding_model = "one_hot"
    #         values["ag"].embedding_model = "one_hot"
    #     if "custom" in node_feat_type:
    #         values["ab"].embedding_model = "custom"
    #         values["ag"].embedding_model = "custom"
    #     if "custom" in node_feat_type or values["ab"].embedding_model == "custom" or values["ag"].embedding_model == "custom":
    #         """
    #         If any `embedding_model` for `ab` or `ag` is set to `custom`, 
    #         ensure `node_feat_type` includes 'custom'.
    #         """
    #         if "custom" not in node_feat_type:
    #             values["node_feat_type"].append("custom")
    #     return values
